...
//        String ressrc = upload(educate_degree_src,request);
    ...
    @ResponseBody
    int updateDegreeInfo(HttpServletRequest request,TEducateDegreeEntity tEducateDegreeEntity,String id){
        int user_id;
        if(StringUtils.isEmpty(id)){
            user_id = (int) request.getSession().getAttribute("user_id");
            System.out.println(1);
        ...
        int res = degreeInfoService.updateDegreeInfo(user_id,degree_id,tEducateDegreeEntity);
        if(res == 200){
            return 200;
        }else{
            return 0;
